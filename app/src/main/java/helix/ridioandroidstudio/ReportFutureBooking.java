/** Ridio v1.0.1
 * 	Purpose	   : Generate reports based on bike category for future dates 
 *  Created by : Abish 
 *  Created Dt : old file
 *  Modified on: 
...
public class ReportFutureBooking extends Activity implements OnClickListener,OnItemSelectedListener {
	...
	String bike_no_from_adapter,what_bike,vendor_id,spvalue,intentvalue,today,selected_veh_category;
...
//    		}
	...
	private void initialize() {
		...
		heading.setText("Future Booking");
	        ...
	        if(temp>0){
				...
				if(temp1>1)
					...
					if(isOnline()){
						...
						new ReportFuture().execute(vendor_id,from_date.getText().toString(),
	        				to_date.getText().toString(),selected_veh_category);
...
//		adapter = new BikeSpinnerAdapter(ReportFutureBooking.this, R.layout.bike_spinner, CustomListViewValuesArr,res);
...
//	}
	...
	private void alertMessageWrongFromDate() {
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(ReportFutureBooking.this);
		...
		alertDialogBuilder.setNegativeButton("No Problem",new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog,int id) {
				...
				if(isOnline()){
					...
					view1.setVisibility(View.VISIBLE);
					new ReportFuture().execute(vendor_id,from_date.getText().toString(),to_date.getText().toString(),selected_veh_category);
				}
				else
					alertInternet();
			}						
	...
	class ReportFuture extends AsyncTask<String, Void, String> {		
		...
		protected void onPreExecute() {
			...
			progressDialog = new ProgressDialog(ReportFutureBooking.this, AlertDialog.THEME_HOLO_LIGHT);
		...
		protected String doInBackground(String... arg0) {
			...
			try{
				...
				postReceiverUrl = "http://phpws.ridio.in/FutureBookingReport";
                ...
                nameValuePairs.add(new BasicNameValuePair("vehicle_category", arg0[i]));
...
//						String b_name=vdb.getVehicleName(_reg);
						...
						String b_name=data.getJSONObject(i).getString("Date");
						b_name=b_name+"/ "+data.getJSONObject(i).getString("Day");
						String ttl_dy=data.getJSONObject(i).getString("BookedBikes");
						String bked_dy=data.getJSONObject(i).getString("AvailableBikes");
						String percent=data.getJSONObject(i).getString("AvailablePercentage");
						String ttl_amt=data.getJSONObject(i).getString("TotalAmount");
						//bike_reg.add(_reg);
						bike_n.add(b_name);
						total_dy.add(ttl_dy);
	...
	public void callMethodRevenueList(){	        		
		list_past_report.setAdapter(new ReportCustomAdapterRevenueDisplay(this,bike_n, total_dy,
